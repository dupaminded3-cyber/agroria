...
async function maakVideo(trekkerId) {
  ...
  try {
    // 1. Foto's voorbereiden: naar vaste 16:9-beelden, anderhalf keer de
    //    videogrootte — genoeg voor een strakke camerabeweging (max. zoom
    //    1,15x), en zuinig genoeg voor een kleine server.
    ...
    for (let i = 0; i < fotos.length; i++) {
      ...
      await sharp(bronPad).rotate()
        .resize(Math.round(BREED * 1.5), Math.round(HOOG * 1.5), { fit: 'cover', position: 'attention' })
        .jpeg({ quality: 90 })
        .toFile(doel);
      beelden.push(doel);
    }
    ...
    for (let i = 0; i < beelden.length; i++) {
      ...
      let filter = `[0:v]zoompan=z='${m.z}':x='${m.x}':y='${m.y}':d=${frames}:s=${BREED}x${HOOG}:fps=${FPS}[shot]`;
      const args = ['-i', beelden[i]];
      if (i === 0) {
        // Titel zacht laten opkomen en verdwijnen over het eerste shot
        args.push('-loop', '1', '-t', FOTO_DUUR.toFixed(2), '-i', titelPng);
        filter += `;[1:v]fps=${FPS},format=rgba,fade=t=in:st=0.5:d=0.5:alpha=1,fade=t=out:st=${(FOTO_DUUR - 0.9).toFixed(2)}:d=0.5:alpha=1[titel]` +
