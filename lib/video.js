...
const VIDEO_VERSIE = 3; // v3: sub-pixel-vloeiende zoom (geen schudden meer)
function bron(t) {
...
// Maximale zoom van de camerabeweging (6%): rustig en nauwelijks opdringerig.
const ZOOM = 1.06;

// De zoombeweging wordt frame voor frame door sharp gerenderd met een echte
// sub-pixel-transformatie (affine + bicubisch). De zoomfilter van ffmpeg kan
// alleen hele pixelstapjes maken en dat oogde als schudden; deze aanpak is
// wiskundig vloeiend. Even nummer = rustig inzoomen, oneven = uitzoomen.
// Een zachte ease-in/ease-out maakt het begin en einde van de beweging kalm.
function zoomOpFrame(i, n, frames) {
  const p = frames <= 1 ? 0 : n / (frames - 1);
  const vloeiend = p * p * (3 - 2 * p); // smoothstep: rustig starten en eindigen
  return i % 2 === 0
    ? 1 + (ZOOM - 1) * vloeiend
    : ZOOM - (ZOOM - 1) * vloeiend;
}

// Rendert alle beeldjes van één shot: de volledige foto, gecentreerd, met een
// heel rustige zoom. bronBuf is de foto op iets groter formaat dan nodig
// (srcW×srcH); elk frame wordt met een affine transformatie (met bicubische
// interpolatie en een fractionele verschuiving) exact op maat gezet.
async function renderShotFrames(bronBuf, srcW, srcH, vw, vh, i, frames, map) {
  for (let n = 0; n < frames; n++) {
    const z = zoomOpFrame(i, n, frames);
    const s = (vw * z) / srcW;             // schaal t.o.v. de bronafbeelding
    const dx = (vw * z - vw) / 2;          // fractioneel: gecentreerd uitsnijden
    const dy = (vh * z - vh) / 2;
    // In twee losse stappen: sharp voert bewerkingen namelijk in een eigen
    // vaste volgorde uit, waardoor extract vóór affine zou lopen als het in
    // één keten staat — met een verkeerd (en per frame wisselend) formaat
    // als gevolg. Tussenstap als raw pixels: geen dubbel codeer-werk.
    ...
    await sharp(raw.data, { raw: { width: raw.info.width, height: raw.info.height, channels: raw.info.channels } })
      .extract({ left: 0, top: 0, width: vw, height: vh })
      .jpeg({ quality: 92 })
...
async function maakVideo(trekkerId) {
  ...
  try {
    // 1. Foto's voorbereiden. De foto wordt NIET bijgesneden naar breedbeeld
    //    (dat maakte het beeld "ingezoomd"): hij komt volledig in beeld, met
    //    links/rechts of boven/onder een zachte, donkere vervaagde versie van
    //    dezelfde foto als achtergrond — zoals bij professionele dealervideo's.
    ...
    for (let i = 0; i < fotos.length; i++) {
      ...
      // De bron voor de zoom: precies groot genoeg voor de maximale zoomstand,
      // zodat elk frame een lichte (kwaliteitsbehoudende) verkleining is.
      const srcW = Math.ceil((vw * ZOOM) / 2) * 2;
      const srcH = Math.ceil((vh * ZOOM) / 2) * 2;
      const voorgrond = await sharp(buf)
      ...
      await sharp(buf)
        ...
        .jpeg({ quality: 80 })
        .toFile(achtergrond);
      beelden.push({ voorgrond, srcW, srcH, achtergrond, vw, vh });
    }
    ...
    for (let i = 0; i < beelden.length; i++) {
      ...
      // Voorgrond: alle beeldjes van de zoombeweging vloeiend renderen
      const framesMap = path.join(tmp, `frames${i}`);
      fs.mkdirSync(framesMap, { recursive: true });
      await renderShotFrames(b.voorgrond, b.srcW, b.srcH, b.vw, b.vh, i, frames, framesMap);
      // Achtergrond: stilstaande vervaagde foto; voorgrond er gecentreerd op
      const args = [
        '-loop', '1', '-t', FOTO_DUUR.toFixed(2), '-i', b.achtergrond,
        '-framerate', String(FPS), '-i', path.join(framesMap, 'f%03d.jpg')
      ];
      let filter =
        `[0:v]fps=${FPS}[bg]` +
      ...
      await ffmpeg(args);
      fs.rmSync(framesMap, { recursive: true, force: true }); // schijf meteen opruimen
      segmenten.push(seg);
    }
